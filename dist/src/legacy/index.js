...
const mountLegacyApp = (app, io) => {
    ...
    app.get('/api/v1/agents', async (req, res) => {
        try {
            ...
            const agents = allUsers.filter(u => ['AGENT', 'ADMIN', 'SUPER_ADMIN', 'SUPERVISOR'].includes(u.role) &&
    ...
    app.post("/webhooks/signalwire", async (req, res) => {
        ...
        if (lowerTranscript.includes('appointment'))
            department = 'appointments';
        else if (lowerTranscript.includes('home care') || lowerTranscript.includes('pricing'))
            department = 'sales';
        let customer = null;
        if (caller) {
    ...
    app.post('/api/messages/send-instagram', async (req, res) => {
        ...
        try {
            ...
            if (!conversation)
                return res.status(404).json({ error: 'Conversation not found' });
            const customerIgId = conversation.customer.phone;
